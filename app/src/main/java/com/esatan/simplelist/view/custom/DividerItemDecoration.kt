package com.esatan.simplelist.view.custom
...
class DividerItemDecoration(
    context: Context,
    orientation: Int,
    private val isContainFistItem: Boolean = false,
    private val isContainLastItem: Boolean = false
) :
    ...
    private fun drawVertical(
        ...
        val last = if (isContainLastItem) childCount else childCount - 1
        for (i in 0 until last) {
            val child = parent.getChildAt(i)
            parent.getDecoratedBoundsWithMargins(child, mBounds)

            if (isContainFistItem && i == 0) {
    ...
    override fun getItemOffsets(
        ...
        if (mDivider == null || parent.layoutManager?.getPosition(view) == state.itemCount - 1) {
            outRect.set(0, 0, 0, 0)
            return
        }
        if (mOrientation == VERTICAL) {
